...
CMD="GhostFS --client --host $NEXTCLOUD_HOST --port $NEXTCLOUD_PORT --cert /apps/ghostfs/secrets/cert.pem -o big_writes -o large_read -o allow_root --write-back 32 --read-ahead 0 --user $HIP_USER --token $HIP_PASSWORD /home/$HIP_USER/nextcloud & echo \$! > /tmp/ghostfs_status"
...
if ! ps -p $(cat /tmp/ghostfs_status) > /dev/null
then
   echo "failed."
   exit 1
fi
