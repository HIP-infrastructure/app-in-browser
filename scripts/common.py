...
def get_hip_config():
    with open("hip.config.yml", encoding="utf-8") as f:
...
def is_build_needed(ci_registry_image, image_name, version, tag, force):
    ...
    if (is_image_in_registry
        ...
        print(
            f"Overwriting {image} found on the registry ('force' option used)")
    elif is_image_in_registry and is_version_latest:
        print(
            f"Overwriting {image} found on the registry ('latest' version used)")
    return True
